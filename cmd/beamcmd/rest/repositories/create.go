...
func HandleCreate(req *http.Request) {
	...
	println("[HandleCreate] Body before conversion:", string(bodyBefore))

	// Convert to old format
	oldFormat := FillOldFormat(input)

	bodyAfter, err := json.Marshal(oldFormat)
	if err == nil {
		// Log after conversion
		println("[HandleCreate] Body after conversion:", string(bodyAfter))
		req.Body = io.NopCloser(bytes.NewReader(bodyAfter))
		req.ContentLength = int64(len(bodyAfter))
	}
}

// FillOldFormat takes a new API format (possibly partial or empty) and returns the fully populated old format as map[string]interface{}.
func FillOldFormat(input map[string]interface{}) map[string]interface{} {
	// Define the full old format template for params
